...
using static DapperInput.Hook;

namespace DapperInput {
	public class Hook {
		#region PInvoke
		...
		public event KeyUpEventDelegate KeyUpEvent;
		...
		private int HookCallback(int code, IntPtr wParam, ref KBDLLHOOKSTRUCT lParam) {
			...
			KeyboardHookEventArgs args = new KeyboardHookEventArgs(lParam);
			...
			try {
				...
				result = args.Cancel ? 1 : CallNextHookEx(IntPtr.Zero, code, wParam, ref lParam);
			}
